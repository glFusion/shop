...
*   @package paypal
*/
class sqtest extends \Paypal\Gateway
{
    ...
    */
    public function __construct()
    {
        ...
        $this->gw_name = 'square';
        $this->gw_desc = 'SquareConnect-Testing';
    ...
    */
    public function gatewayVars($cart)
    {
        ...
        $cartID = $cart->CartID();
        $shipping = 0;
        $this->Currency = \Paypal\Currency::getInstance();

        $accessToken = $this->token;
        $locationId = $this->loc_id;

        $order = $this->gwvars1($cart);
        var_dump($order);die;
        ...
        if ($by_gc > 0) {
            ...
            $PriceMoney->setAmount($this->Currency->toInt($total_amount));
            ...
            foreach ($cart->Cart() as $Item) {
                ...
                $PriceMoney->setAmount($this->Currency->toInt($Item->price));
                $itm = new \SquareConnect\Model\CreateOrderRequestLineItem;
        ...
        if ($shipping > 0) {
            ...
            $ShipMoney->setAmount($this->Currency->toInt($shipping));
            $itm = new \SquareConnect\Model\CreateOrderRequestLineItem;
    ...
    */
    ...
    private function gwvars1($cart)
    {
        ...
        if ($by_gc > 0) {
            ...
            $lineItems[] = array(
                ...
                'base_price_money' => array(
                    'amount'    => $thie->Currency->toInt($total_amount),
            ...
            foreach ($cart->Cart() as $Item) {
                ...
                $itm = array(
                    ...
                    'base_price_money' => array(
                        'amount'    => $this->Currency->toInt($Item->price),
                        'currency'  => $this->currency_code,
                    ),
                );
                ...
                if ($Item->taxable) {
                    $itm['taxes'] = array(
                        ...
                        'percentage' => (string)($_PP_CONF['tax_rate'] * 100),
                        'applied_money' => array(
                            'amount'    => $this->Currency->toInt($tax),
                            'currency'  => $this->currency_code,
                        ),
                    );
                }
        ...
        if ($shipping > 0) {
            $itm = array(
                'name'  => $LANG_PP['shipping'],
                'quantity' => 1,
                'base_price_money' => array(
                    'amount'    => $this->Currency->toInt($shipping),
                    'currency'  => $this->currency_code,
                ),
            );
            array_push($lineItems, $itm);
        ...
        $order = array(
            'idempotency_key'   => $cart->token,
            'order' => array(
                'reference_id'  => $cart->order_id,
